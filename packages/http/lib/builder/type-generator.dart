...
/// Maps class name -> list of fields that form a oneOf union.
/// For these classes a sealed class hierarchy is generated instead of a single
/// class with all fields nullable.  This mirrors the ONEOF_FIELDS map in the
/// JS codegen (sdk-types/scripts/codegen.js).
const Map<String, List<String>> ONEOF_FIELDS = {
...
///
/// The base sealed class contains all non-oneOf fields plus an abstract
/// [toJson]. Each oneOf field becomes a [final class] variant that extends the
/// base and carries that one field as required.  A factory [fromJson] on the
/// base dispatches to the correct variant by checking which key is present.
/// Generates a single class with named constructors for oneOf types.
///
/// Each oneOf field gets a named constructor (e.g. `.oidcToken(...)`) that
/// initialises that field as required and null-initialises all other oneOf
/// fields.  Base fields appear in every constructor.  This mirrors the JS
/// codegen's ONEOF_FIELDS treatment while staying idiomatic Dart.
String generateOneOfClass(
  ...
  for (final f in baseFields) {
    if ((f.description ?? '').isNotEmpty) b.writeln('  /// ${f.description}');
    b.writeln('  final ${f.type} ${f.fieldName};');
  }
...
void generateTypesFromSwagger() {
  ...
  for (final entry in defs.entries) {
    ...
    if (def['type'] == 'object' && def['properties'] is Map) {
      final oneofFields = ONEOF_FIELDS[name];
      if (oneofFields != null) {
        // Generate a sealed class hierarchy for this oneOf type.
        out.write(generateOneOfClass(name, def, defs, oneofFields));
      } else {
