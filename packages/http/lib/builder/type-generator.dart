...
const String COMMENT_HEADER = '/* @generated by codegen. DO NOT EDIT BY HAND */';
...
final String swaggerPath =
    _resolve('../swagger/public_api.swagger.json');
...
String generateEnumHelpers(String enumName, List<dynamic> values) {
  ...
  b.writeln('  switch (value) {');
  for (int i = 0; i < values.length; i++) {
    b.writeln('    case $enumName.${tokens[i]}: return ${jsonEncode(values[i])};');
  }
  b.writeln('  }');
  b.writeln('}');
  b.writeln();
...
String generateClass(
  ...
  final requiredList =
      (def['required'] is List) ? (def['required'] as List).cast<String>() : <String>[];
...
String _jsonReadExpr(
  ...
  if (typeStr == 'String' || typeStr == 'num' || typeStr == 'bool' || typeStr == 'dynamic' || typeStr.startsWith('Map<')) {
  ...
  if (_looksLikeList(typeStr)) {
    ...
    if (inner == 'String' || inner == 'num' || inner == 'bool' || inner == 'dynamic') {
  ...
  return isRequired ? '$typeStr.fromJson($read as Map<String, dynamic>)' : '$read == null ? null : $typeStr.fromJson($read as Map<String, dynamic>)';
}
...
String _jsonWriteExpr(
  ...
  final base = typeStr.endsWith('?') ? typeStr.substring(0, typeStr.length - 1) : typeStr;

  if (base == 'String' || base == 'num' || base == 'bool' || base == 'dynamic' || base.startsWith('Map<')) {
    return nonNull;
  }
...
String generateApiTypes(Map<String, dynamic> swagger, String prefix) {
  ...
  for (final entry in paths.entries) {
    ...
    final responseTypeName = responseRef != null ? refToName(responseRef) : null;
    ...
    if (mType == 'command') {
      ...
      for (final p in params) {
        ...
        if (param['in'] == 'body' &&
          ...
          final reqProps = (reqDef['properties'] as Map?)?.cast<String, dynamic>();
          if (reqProps != null &&
            ...
            final activityTypeKey = (reqProps['type']['enum'] as List).first as String?;
            final mapped = activityTypeKey != null ? VERSIONED_ACTIVITY_TYPES[activityTypeKey] : null;
            if (mapped != null) {
      ...
      b.writeln('  const $apiTypeName({required this.activity, ${resultTypeName != null ? 'this.result,' : ''}});');
      b.writeln('  factory $apiTypeName.fromJson(Map<String, dynamic> json) {');
      b.writeln('    return $apiTypeName(');
      b.writeln('      activity: v1Activity.fromJson(json[\'activity\'] as Map<String, dynamic>),');
      if (resultTypeName != null) {
        b.writeln('      result: json.containsKey(\'result\') && json[\'result\'] != null'
      ...
      if (respDef is Map && respDef['properties'] is Map) {
        ...
        b.writeln('  factory $apiTypeName.fromJson(Map<String, dynamic> json) => const $apiTypeName();');
        b.writeln('  Map<String, dynamic> toJson() => {};');
        b.writeln('}\n');
      }
    } else if (mType == 'activityDecision') {
      final activityType = responseTypeName != null ? defs[responseTypeName] : null;
    ...
    if (requestTypeDef == null) continue;
    ...
    if (mType == 'command' || mType == 'activityDecision') {
      fields.addAll([
        FieldSpec(fieldName: 'timestampMs', jsonKey: 'timestampMs', type: 'String?', nullable: true, description: null),
        FieldSpec(fieldName: 'organizationId', jsonKey: 'organizationId', type: 'String?', nullable: true, description: null),
      ]);
      final props = (requestTypeDef['properties'] as Map?)?.cast<String, dynamic>();
      final ref = props?['parameters'] is Map ? (props?['parameters'][r'$ref'] as String?) : null;
      if (ref != null) {
        final intentTypeName = refToName(ref);
        final intentDef = (defs[intentTypeName] as Map).cast<String, dynamic>();
        final isAllOptional = METHODS_WITH_ONLY_OPTIONAL_PARAMETERS.contains(methodName);
        final reqd = (intentDef['required'] is List)
            ? (intentDef['required'] as List).cast<String>()
            : <String>[];
        final iprops = (intentDef['properties'] as Map?)?.cast<String, dynamic>() ?? {};
        for (final e in iprops.entries) {
          final key = e.key;
          final s = (e.value as Map).cast<String, dynamic>();
          String t = 'dynamic';
          if (s[r'$ref'] is String) t = refToName(s[r'$ref'] as String);
      ...
      fields.add(FieldSpec(fieldName: 'organizationId', jsonKey: 'organizationId', type: 'String?', nullable: true, description: null));
      final reqProps = (requestTypeDef['properties'] as Map?)?.cast<String, dynamic>() ?? {};
    ...
    // fromJson
    bodyBuf.writeln('  factory $apiBodyTypeName.fromJson(Map<String, dynamic> json) {');
    for (final f in fields) {
    ...
    for (final f in fields) {
      ...
      bodyBuf.writeln("    if (${f.fieldName} != null || ${!f.nullable}) { _json['${f.jsonKey}'] = $write; }");
    }
    ...
    b.writeln('  factory $apiInputTypeName.fromJson(Map<String, dynamic> json) => '
...
void main() {
  final swaggerMain = jsonDecode(File(swaggerPath).readAsStringSync()) as Map<String, dynamic>;
  final swaggerAuth = jsonDecode(File(authProxySwaggerPath).readAsStringSync()) as Map<String, dynamic>;
  ...
  for (final entry in defs.entries) {
    ...
    if (def['type'] == 'object' && def['properties'] is Map) {
      ...
      out.writeln('  factory $name.fromJson(Map<String, dynamic> json) => const $name();');
