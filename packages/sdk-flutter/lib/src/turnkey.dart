...
class TurnkeyProvider with ChangeNotifier {
  ...
  /// [organizationId] Optional organization ID. Defaults to the current session’s organization ID.
  ///
  /// Returns a list of [PolicyWithId] containing `policyId`, `policyName`,
  /// `effect`, and optional `condition`, `consensus`, `notes`.
  Future<List<PolicyWithId>> fetchOrCreatePolicies({
    ...
    final List<PolicyWithId> alreadyExisting = [];
    ...
    for (final p in policies) {
      ...
      if (existingId != null) {
        alreadyExisting
            .add(PolicyWithId.fromCreateIntent(p, policyId: existingId));
      } else {
        missing.add(p);
      }
    }
    ...
    final List<PolicyWithId> newlyCreated = [];
    for (var i = 0; i < missing.length; i++) {
      newlyCreated.add(
        PolicyWithId.fromCreateIntent(missing[i], policyId: newIds[i]),
      );
    }
