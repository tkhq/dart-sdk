...
class TurnkeyProvider with ChangeNotifier {
    ...
    try {
      ...
      for (final entry in allSessions.entries) {
        ...
        if (!isValidSession(session)) {
          ...
          // If the active session was expired, clear it from state
        ...
        // Schedule expiration for active sessions
        await _scheduleSessionExpiration(sessionKey, session.expiry);
      }

      // 3. Set allSessions in memory (if you want to track them)
      // (Optional: you could keep a local cache field here)
      ...
      if (activeSessionKey != null) {
        ...
        if (activeSession != null) {
          // 5. Create a client from that session
          ...
          // Optionally refresh user/wallets if you have those APIs
  ...
  Future<String> createApiKeyPair({
    ...
    if (storeOverride) {
      _secureStorageStamper.setPublicKey(publicKey);
    }
  ...
  Future<Session> storeSession({
    ...
    // 3. Create API client using the session's public key + secure storage stamper
    final client = createClient(session.publicKey, config.apiBaseUrl);
    this.client = client;
  ...
  TurnkeyClient createClient(String publicKey, String apiBaseUrl) {
